...
Database: MySQL
ORM: Spring Data JPA
Build Tool: Maven / Gradle (update based on what you used)
...
MethodEndpointDescriptionPOST/api/pollsCreate a new pollGET/api/pollsGet all pollsGET/api/polls/{id}Get a specific poll by ID (includes vote counts)POST/api/polls/voteCast a vote (body: pollId, optionIndex)
...
bash   git clone https://github.com/yourusername/pollpulse.git
   cd pollpulse
...
properties   spring.datasource.url=jdbc:mysql://localhost:3306/pollpulse
...
Concurrency handling on vote counts (current increment logic can lose votes under simultaneous requests)
